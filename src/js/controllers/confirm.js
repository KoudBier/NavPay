...
angular
  ...
  .controller('confirmController', function(
    ...
    var configFeeLevel = walletConfig.settings.feeLevel
      ? walletConfig.settings.feeLevel
      : 'normal';
      ...
      function setWalletSelector(network, minAmount, cb) {
        ...
        lodash.each($scope.wallets, function(w) {
          walletService.getStatus(w, {}, function(err, status) {
            ...
            $scope.insufficientFunds = lodash.isEmpty(filteredWallets);
          ...
          if (sendMaxInfo) {
            ...
            if (tx.sendMax && sendMaxInfo.amount == 0) {
              ...
              popupService.showAlert(
                ...
                gettextCatalog.getString('Not enough funds for fee')
              );
              return cb('no_funds');
            }
    ...
    function setButtonText(isMultisig, isPayPro) {
      $scope.buttonText =
        gettextCatalog.getString(
          isCordova && !isWindowsPhoneApp ? 'Slide' : 'Click'
        ) + ' ';
      ...
      } else $scope.buttonText += gettextCatalog.getString('to send');
    }
    ...
    function showSendMaxWarning(sendMaxInfo) {
        ...
        if (sendMaxInfo.utxosBelowFee > 0) {
          warningMsg.push(
            gettextCatalog.getString(
              ...
              {
                amountBelowFeeStr: txFormatService.formatAmountStr(
        ...
        if (sendMaxInfo.utxosAboveMaxSize > 0) {
          warningMsg.push(
            gettextCatalog.getString(
              ...
              {
                amountAboveMaxSizeStr: txFormatService.formatAmountStr(
      ...
      var msg = gettextCatalog.getString(
        '{{fee}} will be deducted for NavCoin networking fees.',
        {
    ...
    var setSendError = function(msg) {
      ...
      popupService.showAlert(
        gettextCatalog.getString('Error at confirm'),
        bwcError.msg(msg)
      );
    };
    ...
    $scope.openPPModal = function() {
      $ionicModal
        .fromTemplateUrl('views/modals/paypro.html', {
          scope: $scope
        })
        .then(function(modal) {
    ...
    $scope.approve = function(tx, wallet, onSendStatusChange) {
      ...
      if ($scope.paymentExpired) {
        popupService.showAlert(
          null,
          gettextCatalog.getString('This NavCoin payment request has expired.')
        );
        $scope.sendStatus = '';
        $timeout(function() {
          $scope.$apply();
      ...
      getTxp(lodash.clone(tx), wallet, false, function(err, txp) {
        ...
        function confirmTx(cb) {
          ...
          var message = gettextCatalog.getString(
            'Sending {{amountStr}} from your {{name}} wallet',
            {
          ...
          popupService.showConfirm(null, message, okText, cancelText, function(
            ok
          ) {
            return cb(!ok);
          });
        ...
        function publishAndSign() {
          if (!wallet.canSign() && !wallet.isPrivKeyExternal()) {
            ...
            return walletService.onlyPublish(
              wallet,
              txp,
              function(err) {
    ...
    $scope.chooseFeeLevel = function(tx, wallet) {
      ...
      $ionicModal
        .fromTemplateUrl('views/modals/chooseFeeLevel.html', {
          scope: scope,
      ...
      scope.hideModal = function(newFeeLevel, customFeePerKB) {
        ...
        $log.debug(
          'New fee level choosen:' + newFeeLevel + ' was:' + tx.feeLevel
        );
