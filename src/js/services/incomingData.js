  ...
  .factory('incomingData', function(
    ...
    root.redir = function(data, privatePayment, stopRedirect) {
      ...
      if (bitcore.URI.isValid(data)) {
        ...
        bitcore.Address.isValid(data, 'livenet') ||
        bitcore.Address.isValid(data, 'testnet')
      ) {
        ...
        data &&
        data.indexOf(appConfigService.name + '://glidera') === 0
      ) {
        ...
        data &&
        data.indexOf(appConfigService.name + '://coinbase') === 0
      ) {
        var code = getParameterByName('code', data);
        $ionicHistory.nextViewOptions({
          disableAnimate: true
        ...
        data &&
        (data.substring(0, 2) == '6P' || checkPrivateKey(data))
      ) {
        root.showMenu({
          data: data,
          type: 'privateKey'
